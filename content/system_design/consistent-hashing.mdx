---
id: 2023-01-18-consistent-hashing
title: consistent hashing
metaTitle: Consistent Hashing | Anas's Digital Garden
metaDescription: consistent hashing | Anas's Digital Garden
createdDate: 2023-01-18
updatedDate: 2023-01-18
weekNumber: 202303
category: system_design
tags: ["system_design", "consistent-hashing"]
---
...
![Untitled](./consistent_hashing/Untitled.png)

he scheme described in the above diagram solves the problem of finding a server for storing/retrieving the data. **But when we add or remove a server, all our existing mappings will be broken.**

This is because the total number of servers will be changed, which was used to find the actual server storing the data. So to get things working again, we have to **remap all the keys** and move our data based on the new server count, which will be a **complete mess!**
...
![Consistent Hashing ring](./consistent_hashing/untitled-1.png)
...
- This means that each node will be assigned one token.
...
![Untitled](./consistent_hashing/untitled-2.png)
...
![Untitled](./consistent_hashing/untitled-3.png)
...
- For example, when a node is removed, the next node becomes responsible for all of the keys stored on the outgoing node.
...
- Existing nodes can die and may need to be decommissioned.
- Similarly, new nodes may be added to an existing cluster to meet growing demands.
...
![Untitled](./consistent_hashing/untitled-4.png)
...
![Untitled](./consistent_hashing/untitled-5.png)
...
1. As Vnodes help spread the load more evenly across the physical nodes on the cluster by dividing the hash ranges into smaller subranges, this speeds up the rebalancing process after adding or removing nodes.
    1. When a new node is added, it receives many Vnodes from the existing nodes to maintain a balanced cluster.
    2. Similarly, when a node needs to be rebuilt, instead of getting data from a fixed number of replicas, many nodes participate in the rebuild process.
...
![Untitled](./consistent_hashing/untitled-6.png)

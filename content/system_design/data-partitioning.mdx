---
id: 2023-01-18-data-partitioning
title: data partitioning
metaTitle: Data Partitioning | Anas's Digital Garden
metaDescription: data partitioning | Anas's Digital Garden
createdDate: 2023-01-18
updatedDate: 2023-01-18
weekNumber: 202303
category: system_design
tags: ["system_design", "data-partitioning", "Database"]
---
...
- In the previous example, splitting locations based on their zip codes assume that places will be evenly distributed across the different zip codes. This assumption is not valid as there will be a lot of places in a thickly populated area like Manhattan as compared to its suburb cities.
...
- A workaround for this problem is to use [Consistent Hashing](Consistent%20Hashing.md).
...
- This is a very simple strategy that ensures uniform data distribution.
...
- A common workaround for this problem is to denormalise the database so that queries that previously required joins can be performed from a single table.
- Of course, the service now has to deal with denormalisation's perils, such as data inconsistency.
...
- This means, applications that require referential integrity on partitioned databases often have to enforce it in application code.

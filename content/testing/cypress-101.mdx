...
In other cases, we may not want to modify the behavior of our functions, but we are still interested in them.

For example, we want to test whether they were called, if they were called with the right arguments, or how many times they were called. This is where spies are useful. We can spy on a function in the following ways:
...
```javascript
cy.get('@downloadButton').click();cy.get('@downloadButton').should('be.disabled');
```

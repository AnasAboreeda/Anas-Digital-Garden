---
id: 2023-01-18-databases
title: databases
metaTitle: Databases | Anas's Digital Garden
metaDescription: databases | Anas's Digital Garden
createdDate: 2023-01-18
updatedDate: 2023-01-18
weekNumber: 202303
category: interview_preparation/interview_questions
tags: ["interview_questions", "databases"]
---
...
- Write sql statement to identify the duplicate records in a table, how to delete the duplication, and how to force uniqueness on the table's EmployeeID column for avoiding later duplication?

	```python
	Employee table

	EmplyeeID	EmployeeName		EmployeeAddress	EmployeeTitle

	123		         John Smith		                abc, NYC		sr.developer
	123		        John Smith		                abc, NYC		sr.developer
	234         George Stanely                   xyz,PHIL    developer
	```

- Write sql statement to compare the same table in two different DB servers to identify the discrepancy (both ways) based on a column; id. (ideally find the extra records in prod not in dev, and extra records in dev not in prod)
...
-  Export data from table in insert format

	ID,ACCESSIONNUMBER,PATENTUPDATE,PATENTCOUNTRY,PATENTEENAME

	i.e. Insert into derwent_data(ID,ACCESSIONNUMBER,PATENTUPDATE,PATENTCOUNTRY,PATENTEENAME) values ('120','2002206261442','December 1, 2021','USA','US');
...
A: Fuzzy search allows Elasticsearch to return positive matches for non-exact matches of the term you are searching for. This is especially beneficial for eCommerce retailers where site visitors may often have typos in their spelling when trying to locate a product they wish to purchase. Fuzzy match results mean that these visitors are not served with a blank page which would often lead to a user being less likely to convert.
